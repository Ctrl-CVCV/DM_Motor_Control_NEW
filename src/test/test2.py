...
def main():
    ...
    try:
        while True:
            ...
            cv2.line(frame, (center_x - 15, center_y), (center_x + 15, center_y), (150, 150, 150), 2)
            cv2.line(frame, (center_x, center_y - 15), (center_x, center_y + 15), (150, 150, 150), 2)
            ...
            if not in_scanning_round:
                if len(detections) > 0:
                    ...
                    cv2.putText(frame, "IDLE: Waiting for objects...", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            ...
            if in_scanning_round:
                if target_index >= round_target_count:
                    ...
                    cv2.putText(frame, "ROUND COMPLETE! MEMORY SAVED.", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
                    cv2.imshow("RDKX5 Sequential Targeting", frame)
                    cv2.waitKey(3000)
                    in_scanning_round = False
                    ...
                    if len(detections) > 0:
                        ...
                        if len(scanned_history) == target_index:
                            scanned_history.append(obj_name)
                            print(f"👉 [锁定] 第 {target_index + 1}/{round_target_count} 个物体: {obj_name}")
                            info_mcu.send_class_info(obj_name)
                        ...
                        cv2.line(frame, (center_x, center_y), (int(cx), int(cy)), (0, 255, 255), 2)
                        cv2.putText(frame, f"TARGET {target_index+1}/{round_target_count}: {obj_name}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                        cv2.putText(frame, f"ERR: dx={dx_comp:.2f} dy={dy_comp:.2f} (SMOOTH)", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
                        cv2.putText(frame, f"TIME LEFT: {time_left:.1f}s", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        ...
                        mcu.send_target_data(0, 0, 0)
                        cv2.putText(frame, f"TARGET {target_index+1} LOST! WAITING...", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

            for i, det in enumerate(detections):
                x1, y1, x2, y2 = det["bbox"]
                pt1, pt2 = (int(x1), int(y1)), (int(x2), int(y2))
                active_idx = min(target_index, len(detections) - 1) if in_scanning_round else -1
                color = (0, 0, 255) if (i == active_idx) else (0, 255, 0)
                thickness = 3 if (i == active_idx) else 1
                cv2.rectangle(frame, pt1, pt2, color, thickness)
                cv2.putText(frame, f"#{i+1}", (pt1[0], pt1[1]-5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

            if mcu.ser is None:
                cv2.putText(frame, "MOT_MCU: DISCONNECT", (10, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            elif mcu.is_mcu_alive():
                cv2.putText(frame, f"MOT_MCU [{mcu.port}]: ONLINE", (10, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            else:
                cv2.putText(frame, f"MOT_MCU [{mcu.port}]: WAIT REPLY...", (10, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)

            if info_mcu.ser and info_mcu.ser.is_open:
                cv2.putText(frame, f"UI_MCU [{info_mcu.port}]: ONLINE", (10, frame_h - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            else:
                cv2.putText(frame, f"UI_MCU [/dev/ttyUSB0]: DISCONNECT", (10, frame_h - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
