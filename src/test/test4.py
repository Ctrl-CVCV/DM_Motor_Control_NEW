    ...
    try:
        while True:
            ...
            if target_index >= round_target_count:
                ...
                cv2.putText(frame, "ALL TASKS COMPLETED!", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 3)
                cv2.imshow("RDKX5 Playback Mode", frame)
            ...
            if not target_locked:
                ...
                if matched_det is not None:
                    ...
                    # 还没找到，云台停转，等待目标进入视野
                    mcu.send_target_data(0, 0, 0)
                    cv2.putText(frame, f"SEARCHING: {expected_target_name} ({target_index+1}/{round_target_count})", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
            ...
            if target_locked:
                ...
                # UI 绘制锁定态
                cv2.rectangle(frame, (int(locked_bbox[0]), int(locked_bbox[1])), (int(locked_bbox[2]), int(locked_bbox[3])), (0, 0, 255), 3)
                cv2.line(frame, (center_x, center_y), (int(locked_cx), int(locked_cy)), (0, 255, 255), 2)
                cv2.putText(frame, f"LOCKED: {expected_target_name} ({target_index+1}/{round_target_count})", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(frame, f"ERR: dx={dx_comp:.2f} dy={dy_comp:.2f} (BLIND TRACK)", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
                cv2.putText(frame, f"TIME LEFT: {time_left:.1f}s", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            ...
            for det in detections:
                x1, y1, x2, y2 = det["bbox"]
                # 如果这个框跟我们锁定的框不同，画成绿的
                if not target_locked or det.get("bbox") != locked_bbox:
                    cid = det["class_id"]
                    name = class_names[cid] if cid < len(class_names) else f"OBJ_{cid}"
                    cv2.rectangle(frame, (int(x1), int(y1)), (int(x2), int(y2)), (0, 255, 0), 1)
                    cv2.putText(frame, f"{name}", (int(x1), int(y1)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

            # --- 串口连接状态 UI ---
            if mcu.ser is None:
                cv2.putText(frame, "MOT: DISCONNECT", (10, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
            elif mcu.is_mcu_alive():
                cv2.putText(frame, f"MOT: ONLINE", (10, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            else:
                cv2.putText(frame, f"MOT: WAIT REPLY", (10, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)

            if info_mcu.ser and info_mcu.ser.is_open:
                cv2.putText(frame, "UI: ONLINE", (200, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            else:
                cv2.putText(frame, "UI: DISCONNECT", (200, frame_h - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
